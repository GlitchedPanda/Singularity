...
NTSTATUS dxe_driver::SetSystemEnvironmentPrivilege(BOOLEAN Enable, PBOOLEAN WasEnabled)
{
...
void dxe_driver::SendCommand(MemoryCommand* cmd)
{
...
bool dxe_driver::MemCopy(uint64_t destination, uint64_t source, uint64_t size)
{
...
bool dxe_driver::SetMemory(uint64_t address, uint32_t value, uint64_t size)
{
...
bool dxe_driver::ReadMemory(uint64_t address, void* buffer, uint64_t size)
{
	return MemCopy(reinterpret_cast<uint64_t>(buffer), address, size);
}

bool dxe_driver::WriteMemory(uint64_t address, void* buffer, uint64_t size)
{
	return MemCopy(address, reinterpret_cast<uint64_t>(buffer), size);
}

uint64_t dxe_driver::AllocatePool(nt::POOL_TYPE pool_type, uint64_t size)
{
	if (!size)
		return 0;
	...
	if (!kernel_ExAllocatePool)
		kernel_ExAllocatePool = GetKernelModuleExport(utils::get_kernel_module_address("ntoskrnl.exe"), "ExAllocatePool");
	...
	data[1] = static_cast<uintptr_t>(pool_type);
	data[2] = size;
...
bool dxe_driver::FreePool(uint64_t address)
{
	if (!address)
		return 0;
	...
	if (!kernel_ExFreePool)
		kernel_ExFreePool = GetKernelModuleExport(utils::get_kernel_module_address("ntoskrnl.exe"), "ExFreePool");
...
uint64_t dxe_driver::GetKernelModuleExport(uint64_t kernel_module_base, const std::string& function_name)
{
...
// Call a kernel function (DriverEntry etc.)
bool dxe_driver::CallKernelFunction(NTSTATUS* out_status,
	uint64_t address,
	uint64_t param1,
	uint64_t param2)
{
	...
	uintptr_t data[10] = { 0 };
	data[0] = address;
	data[1] = reinterpret_cast<uintptr_t>(out_status);
	data[2] = param1;
	data[3] = param2;
...
bool dxe_driver::GetNtGdiDdDDIReclaimAllocations2KernelInfo(uint64_t* out_kernel_function_ptr, uint64_t* out_kernel_original_function_address)
{
	...
	{
		const uint64_t kernel_NtGdiDdDDIReclaimAllocations2 = GetKernelModuleExport(utils::get_kernel_module_address("win32kbase.sys"), "NtGdiDdDDIReclaimAllocations2");
...
bool dxe_driver::GetNtGdiGetCOPPCompatibleOPMInformationInfo(uint64_t* out_kernel_function_ptr, uint8_t* out_kernel_original_bytes)
{
	...
	{
		const uint64_t kernel_NtGdiGetCOPPCompatibleOPMInformation = GetKernelModuleExport(utils::get_kernel_module_address("win32kfull.sys"), "NtGdiGetCOPPCompatibleOPMInformation");
...
bool dxe_driver::ClearMmUnloadedDrivers(HANDLE device_handle)
{
	ULONG buffer_size = 0;
	void* buffer = nullptr;
